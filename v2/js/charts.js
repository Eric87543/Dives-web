...
App.Charts = (function () {
  ...
  function trend(container, points, opts) {
    ...
    for (const t of ticks) {
      const y = yAt(t);
      svg += `<line x1="${padL}" y1="${y}" x2="${padL + chartW}" y2="${y}" stroke="${t === 0 ? 'rgba(255,255,255,0.25)' : 'rgba(255,255,255,0.08)'}" stroke-width="1" ${t === 0 ? '' : 'stroke-dasharray="3 3"'}/>`;
      svg += `<text x="${padL - 6}" y="${y + 3}" text-anchor="end" font-size="9" fill="#8a8a8e">${fmtAxis(t)}</text>`;
    }
    ...
    if (opts.xLabels) for (const e of opts.xLabels)
      svg += `<text x="${xAt(e.idx)}" y="${H - 6}" text-anchor="middle" font-size="9" fill="#8a8a8e">${e.label}</text>`;
  ...
  function bars(container, items, opts) {
    ...
    items.forEach((it, i) => {
      ...
      svg += `<text x="${cx.toFixed(1)}" y="${H - 8}" text-anchor="middle" font-size="9" fill="#8a8a8e">${it.label}</text>`;
    });
    svg += `</svg>`;
