...
App.Sync = (function () {
  ...
  const DATA_KEYS = ['d2_accounts', 'd2_snapshots'];
  const FILENAME = 'dives2-networth.json';
  const K = { token: 'd2_sync_token', gist: 'd2_sync_gist', localTs: 'd2_sync_local_ts' };
  ...
  function hasLocalData() {
    try { return (JSON.parse(localStorage.getItem('d2_accounts') || '[]')).length > 0; }
